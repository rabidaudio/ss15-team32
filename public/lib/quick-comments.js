...
riot.tag('comment', '<div class="qc-comment" id="comment/{id}"> <div class="avatar"> <a href="{author.url}"><img src="{author.avatar}"></a> </div> <div class="qc-header"> <a href="{author.url}" class="author text-muted">{author.name}</a> |  <a href="#comment/{id}" class="timestamp text-muted" title="{new Date(opts.data.time).toLocaleString()}">{vagueTime}</a> <a href="#" role="button" class="edit text-muted" if="{false}">Edit</a> </div> <div class="qc-body"> <p each="{p in paragraphs}">{p}</p> </div> <hr></hr> </div>', function(opts) {
...
riot.tag('newcomment', '<div class="qc-comment qc-new"> <fieldset>      <textarea __disabled="{parent.Auth.loggedIn() ? undefined : true}" rows="{height}" class="{gc-new-body: 1, form-control: parent.opts.bootstrap}" name="body" onfocus="{grow}" onblur="{shrink}" onkeydown="{update}" placeholder="{parent.Auth.loggedIn() ? \'Leave a comment\' : \'Sign in to post a comment.\'}"></textarea>\n <div class="qc-user qc-logged-in" if="{parent.Auth.loggedIn()}"> <p>Logged in as {parent.Auth.currentUser().name} (via {capitalize(parent.Auth.currentUser().provider)}). <a href="#" role="button" onclick="{logout}"> Log out or switch accounts</a> </p> </div> <div class="{qc-signin:1, input-group-btn: parent.opts.bootstrap}"> <button class="{qc-logged-out: 1, btn: parent.opts.bootstrap, btn-default: parent.opts.bootstrap, dropdown-toggle: parent.opts.bootstrap}" data-toggle="dropdown" if="{!parent.Auth.loggedIn()}">Sign in<span class="caret"></span></button> <ul class="{qc-login-opts: 1, dropdown-menu: parent.opts.bootstrap, dropdown-menu-right: parent.opts.bootstrap}" role="menu"> <li each="{name, val in parent.Auth.providers}" if="{val.available}"> <a href="#" role="button" onclick="{parent.login}" class="provider-{name}">{parent.capitalize(name)}</a> </li> </ul> </div> <button __disabled="{this.body.value.length ? undefined : true}" class="{submit: 1, btn: parent.opts.bootstrap, btn-primary: parent.opts.bootstrap}" name="submit" onclick="{send}" if="{parent.Auth.loggedIn()}">Submit</button> </fieldset> <hr></hr> </div>', function(opts) {
  this.height = 1
